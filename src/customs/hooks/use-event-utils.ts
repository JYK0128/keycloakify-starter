...
export function useEventUtils() {
  ...
  const prevent = <T extends BaseSyntheticEvent>(callbackLike?: T | Dispatch<T>): Dispatch<T> | ReturnType<Dispatch<T>> => {
  ...
  const stop = <T extends BaseSyntheticEvent>(callbackLike?: T | Dispatch<T>): Dispatch<T> | ReturnType<Dispatch<T>> => {
    if (callbackLike instanceof Function) {
      return (evt) => {
        evt?.stopPropagation();
        callbackLike?.(evt);
