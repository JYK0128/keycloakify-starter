import { DataTableCell } from '#/customs/components/DataTableCell';
import { TableRow } from '#/shadcn/components/ui';
import { cn, cv } from '#/shadcn/lib/utils';
...
export const DataTableRow = forwardRef(
  function DataTableRowInner<T>(
    ...
    const commonStyles = useMemo(() => cn(
      'hover:bg-blue-200',
      row.getCanMultiSelect() === false ? `cursor-pointer` : '',
    ), [row]);
...
function getRowPinningStyles<T>(row: Row<T>, state: TableState) {
  ...
  const styles = cv((key: 'top' | 'bottom', row: Row<T>) => ({
    top: cn(
      row.getPinnedIndex() === (defaultTo(state.rowPinning.top?.length, 0) - 1) ? 'shadow-[-4px_0_4px_-4px_gray_inset]' : '',
      `sticky z-10 bg-background`,
    ),
    bottom: cn(
      row.getPinnedIndex() === (defaultTo(state.rowPinning.bottom?.length, 0) - 1) ? 'shadow-[4px_0_4px_-4px_gray_inset]' : '',
      `sticky z-10 bg-background`,
    ),
  }[key]));
